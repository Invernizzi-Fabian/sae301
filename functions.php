...
function custom_menu_items($items, $args) {
    ...
    if (is_user_logged_in()) {
        // Enlève le lien de connexion s'il est présent
        $items = preg_replace('/<li><a href=".?wp-login.php.?">Connexion<\/a><\/li>/', '', $items);
        
        // Enlève le lien d'inscription s'il est présent
        $items = preg_replace('/<li><a href=".?\/inscription.?">Inscription<\/a><\/li>/', '', $items);

        // Ajoute le lien de déconnexion
        $items .= '<li class="header-menu-item"><a href="' . wp_logout_url(home_url()) . '">Déconnexion</a></li>';
    } else {
        // Ajoute le lien de connexion
        $items .= '<li class="header-menu-item"><a href="' . wp_login_url() . '">Connexion</a></li>';

        // Ajoute le lien d'inscription
        $items .= '<li class="header-menu-item"><a href="' . site_url('/inscription') . '">Inscription</a></li>';
    }

    // Retourne les éléments du menu avec les modifications
    return $items;
}
